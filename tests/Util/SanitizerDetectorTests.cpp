...
TEST(SanitizerDetectorTests, DetectsASanHeapBufferOverflow) {
    std::string stderrOutput =
        "=================================================================\n"
        "==12345==ERROR: AddressSanitizer: heap-buffer-overflow on address 0xdeadbeef at pc 0x000000400080 bp 0x7ffd9778e6a0 sp 0x7ffd9778e698\n"
...
TEST(SanitizerDetectorTests, DetectsUSanIntegerOverflow) {
    std::string stderrOutput =
        "/app/example.cpp:20:10: runtime error: signed integer overflow: 2147483647 + 1 cannot be represented in type 'int'\n"
...
TEST(SanitizerDetectorTests, DetectsASanStackBufferOverflow) {
    std::string stderrOutput =
        "SUMMARY: AddressSanitizer: stack-buffer-overflow /path/to/another.c:30 in foo\n"
...
TEST(SanitizerDetectorTests, MixedContentWithSanitizerError) {
     std::string stderrOutput =
        "Some initial debug output from the program.\n"
